...
for(const [i, movement] of movements.entries()){
  if(movement > 0){
    ...
    console.log(`Movement ${i + 1} : You withdraew ${Math.abs(movement)}`);
  }
}
...
movements.forEach(function(mov, i, arr) {
   if (mov > 0) {
     ...
     console.log(`Movement ${i + 1} : You withdraew ${Math.abs(mov)}`);
   }
});
...
//const maxV = movements.reduce((acc, mov) => acc > mov : acc ? mov)
...
const convertTitleCase = function(title){
  ...
  const exceptitions = ['a', 'an','and', 'the', 'but', 'or', 'on', 'in', 'with'];
  const titleCase = title.toLowerCase().split(' ').map(word => (exceptitions.includes(word) ? word :capitalize(word))).join(' ');
